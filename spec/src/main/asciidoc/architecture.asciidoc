...
Also the method must be implemented as a non-blocking fashion.
...
Message<O> method()
----
| Produces a infinite streams of `Message` associated with the channel `channel`. 
...
O method()
----
| Produces an infinite streams of _payload_ associated with the channel `channel`. Produced payloads are mapped to `Message<O>` by the Reactive Messaging implementation.
|	This method is called for each _request_ made by the subscriber.
...
----
| Produces a infinite streams of `Message` associated with the channel `channel`. The result is a `CompletionStage`. The method cannot be called until the `CompletionStage` returned previously is completed.
|	This method is called for each _request_ made by the subscriber.
...
----
| Produces a infinite streams of _payload_ associated with the channel `channel`. Produced payloads are mapped to `Message<O>` by the Reactive Messaging implementation. The result is a `CompletionStage`. The method cannot be called until the `CompletionStage` returned previously is completed.
|	This method is called for each _request_ made by the subscriber.
...
| The method is called only once to retrieve the `Subscriber` object at assembly time. 
The created `subscriber` is connected to the matching channel.
...
| This method is called for every `Message<I>` instances transiting on the channel `channel`. 
The payload is automatically extracted from the inflight messages using `Message.getPayload()`.
...
| This method is called for every `Message<I>` instances transiting on the channel `channel`. 
The user method is never called concurrently and so the returned `CompletionStage` must be completed before being calling the method with the next `Message`.
...
| This method is called for every incoming message. Never concurrently, so the stream returned previously must complete before having the method invoked with the next incoming `Message`.
...
If the returned `Publisher` emits several elements, these elements are _flatten_ in the outgoing stream as a concatenation of elements.
The Reactive Messaging implementation must create new `Message` instances for each emitted _payloads_ as well as extracing the payload for each incoming `Message`.
| This method is called for every incoming message. Never concurrently, so the stream returned previously must complete before having the method invoked with the next incoming `Message`.
...
The returned `PublisherBuilder` can be empty or emits multiple `Message` instances.
If the returned `PublisherBuilder` emits several elements, these elements are _flatten_ in the outgoing stream as a concatenation of elements.
| This method is called for every incoming message. Never concurrently, so the stream returned previously must complete before having the method invoked with the next incoming `Message`.
...
The returned `PublisherBuilder` can be empty or emits multiple _payload_ instances.
If the returned `PublisherBuilder` emits several elements, these elements are _flatten_ in the outgoing stream as a concatenation of elements.
The Reactive Messaging implementation must create new `Message` instances for each emitted _payloads_ as well as extracing the payload for each incoming `Message`.
| This method is called for every incoming message. Never concurrently, so the stream returned previously must complete before having the method invoked with the next incoming `Message`.
...
| This method is called for every incoming message. Never concurrently, so the previous call must have returned before calling it with the next incoming `Message`.
...
| This method is called for every incoming message. Never concurrently, so the previous call must have returned before calling it with the next incoming _payload_.
...
| This method is called for every incoming message. Never concurrently, so the previous call must have returned and produced a `Message` before calling it with the next incoming `Message`.
...
| This method is called for every incoming _payload_. Never concurrently, so the previous call must have returned and produced a _payload_ before calling it with the next _payload_.
...
----
| Transforms the incoming `Publisher` of `Message`. This method is used to manipulate streams and apply stream transformations.
...
----
| Transforms the incoming `PublisherBuilder` of `Message`. This method is used to manipulate streams and apply stream transformations.
| This method is called once, at _assembly_ time.
...
----
| Transforms the incoming `Publisher` of _payload_. This method is used to manipulate streams and apply stream transformations.
| This method is called once, at _assembly_ time.
...
----
| Transforms the incoming `PublisherBuilder` of _payload_. This method is used to manipulate streams and apply stream transformations.
| This method is called once, at _assembly_ time.
...
Acknowledgement is controlled by the `org.eclipse.microprofile.reactive.messaging.Acknowledgment` annotation. 
The annotation allows configuring the acknowledgement strategy among:

* `NONE` - no acknowledgment is performed
* `MANUAL` - the user is responsible for the acknowledgement, so the Reactive Messaging implementation does not apply implicit acknowledgement
...
Each method signatures has different defaults and accept a different set of strategy.
...
Post-Processing can be optionaly supported by implementations, however it requires a 1:1 mapping between the incoming message and the outgoing message.
...
Invalid acknowledgement policies must be detected and a `DeploymentException` raised when the application starts.
...
The implementation is reponsible to enforce the acknowledgement strategy defined by the user when the `@Acknowledgement` policy is used. 
