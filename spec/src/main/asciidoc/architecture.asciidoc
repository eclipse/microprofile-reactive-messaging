...
----
1. the property value is the fully qualified class name of the `MessagingProvider` implementation to be used for the `my-kafka-topic` channel.

The configuration format is detailed later in this document.

The Reactive Messaging implementation is responsible for finding the connector implementation associated with the given `MessagingProvider` in the user's configuration.
...
Both interfaces contain a `type()` method returning a `MessagingProvider`.
...
If an attribute appears for both a channel and its relevant connector, the connector specific value will be used.
...
----
 ...
 mp.messaging.connector.org.eclipse.reactive.sample.kafka.Kafka.bootstrap-servers=localhost:9092
---- 

Note that as classnames are expected to begin with an uppercase letter and package names are expected to be lowercase, and properties are case sensitive,
the problem of having one fully qualified connector name being the prefix of another is not expected to occur. In practical terms the first uppercased
property name element in a connector property name containing full stops can be read as the class name and the next element as the attribute name.
