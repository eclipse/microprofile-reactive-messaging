...
Reactive Messaging connects matching `@Incoming` and `@Outgoing` running inside the same application.
Besides, it maps specific _channels_ to _external_ technologies such as Apache Kafka, MQTT, Web Sockets, or JMS. 
It means that Reactive Messaging can receive messages from virtually any messaging technology and dispatch messages to any messaging technology.
The component responsible for this role is called _connector_. 
...
* act as a _Publisher_, meaning it retrieves or receives messages from an external messaging technology. The messages will then be sent to a method annotated with `@Incoming`
* act as a _Subscriber_, meaning it dispatches messages to an external messaging technology. The messages are received from a method annotated with `@Outgoing`
...
Both interfaces contains a `type()` method returning a `MessagingProvider`.
`MessagingProvider` is a marker interface implemented by the connector allowing the user to indicate which connector needs to be used. 
For instance, the following `MessagingProvider` implementation can be used in a Kafka connector:
...
public class Kafka implements MessagingProvider {
...
Once defined, the user can, in the configuration, refer to this connector using the fully qualified name of the class.
When the Reactive Messaging implementation processes the configuration, it determines the connector to be used based on the `type` attribute.
...
* which connectors are used (using the `type`) attribute
* the configuration for each channel

The builder methods defined in the `IncomingConnectorFactory` and `OutgoingConnectorFactory` receives a `org.eclipse.microprofile.config.Config` as parameter.
The `Config` object contains key-value pairs to configure the connector.
The configuration is specific to the connector.
Typically, a Kafka connector expects a _bootstrap servers_ entry as well as a _topic_ entry.
...
1. The `type` attribute value is read, and the connector implementation identified. If no connector matches, the deployment must be failed with a `DeploymentException`;
...
[source]
----
 mp.messaging.incoming.my-channel.type=org.eclipse.reactive.sample.kafka.Kafka
...
3. Searches for the `type` attribute and finds `org.eclipse.reactive.sample.kafka.Kafka`
4. Looks for a bean implementing the `IncomingConnectorFactory` interface and having a `type()` method returning the `org.eclipse.reactive.sample.kafka.Kafka` class. If the configuration had contained a `mp.messaging.outgoing.my-channel...` entry, a bean implementing the `OutgoingConnectorFactory` interface would have been searched.
5. Creates a new `Config` object with just the `key=value` pairs:
+
[source]
----
