...
This qualifier defined the `MessagingProvider` associated with the connector.
`MessagingProvider` is a marker interface allowing the user to indicate which connector needs to be used.
...
Implementation would use the following qualifier:
...
4. Looks for a bean implementing the `IncomingConnectorFactory` interface qualified with `@Connector(org.eclipse.reactive.sample.kafka.Kafka.class)`.
