...
Leading-edge applications are capable of dynamic and adaptive capabilities aiming to provide _responsive_ system.
...
This temporal decoupling implies that communication is allowed to occur regardless of when the parties involved in the communication are running, whether they are loaded or overloaded, and whether they are successfully processing messages or failing.
...
Therefore, systems of microservices relying on HTTP tend to be _fragile_, and failures limit their availability.
...
By infusing asynchronous messaging passing at the core of the system, applications enforcing the reactive system's characteristics are inherently elastic and resilient.

image::reactive-system.png[Reactive System Characteristics, 400, 400]

Microservices part of a reactive systems interacts using _messages_.
The location and temporal decoupling, promoted by this interaction mechanism, exhibits numerous benefits such as:

* Better failure management as the temporal decoupling would avoid message-loss and avoid cascading failures.
* Improved elasticity as under fluctuating load the system can decide to scale up and down some of the microservices.
* The ability to introduce new feature quickly as every component receives and publishes messages.
...
However, modern message brokers, popular in microservice deployments, such as Apache Kafka, Amazon Kinesis and Azure Event Hubs, do not support XA transactions, rather, message acknowledgment is handled using offsets with at least once delivery guarantees.
...
MicroProfile Reactive Messaging aims to provide a way to implement _event-driven_ microservices using MicroProfile. 
The key characteristics of the specification make it versatile and perfectly suited to build different types of architecture and applications.
